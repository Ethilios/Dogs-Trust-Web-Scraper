...
DT_base_url = "https://www.dogstrust.org.uk"
# URL includes the filter 'May live with... Dogs'
DT_filtered_page = DT_base_url + "/rehoming/dogs/filters/~~~~~n~~d/page/"
...
current_page = requests.get(DT_filtered_page + "1")
...
dog_selector = "#BodyContent_DogList1_rptDogList_lnkDog_"
...
class Dog:
    def __init__(self, name, breed, location, link):
        self.name = name
        self.breed = breed
    ...
    def __iter__(self):
        return iter([self.name, self.breed, self.location, self.link])

def parse_and_save(index):
    selector = dog_selector + str(index)

    dog_name = soup.select(selector + "> h3")[0].text.strip()
    dog_breed = soup.select(selector + "> span")[0].text.strip()
    dog_location = soup.select(selector + "> span > strong")[0].text.strip()
    relative_link = soup.select(selector)[0].attrs['href']
    dog_link = DT_base_url + relative_link

    # Populate new Dog object and add to all_dogs list
    current_dog = Dog(dog_name, dog_breed, dog_location, dog_link)
...
if (req_status == 200):
    ...
    for i in range(1,num_pages):
        print("\nPAGE: " + str(i))
        current_page = requests.get(DT_filtered_page + str(i))
        soup = BeautifulSoup(current_page.content, 'html.parser')
    ...
    last_page = requests.get(DT_filtered_page + str(num_pages))
